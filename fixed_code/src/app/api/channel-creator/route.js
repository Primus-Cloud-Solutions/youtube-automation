...
export async function POST(request) {
  try {
    ...
    // Connect to an existing YouTube channel
    if (action === 'connect-channel') {
      ...
      try {
        // Validate the API key by fetching user channels
        const result = await youtubeApi.getUserChannels(apiKey);
        
        if (!result.success) {
          return createApiError(result.error || 'Failed to get user channels with provided API key', 500);
        }
        
        if (!result.channels || result.channels.length === 0) {
          return createApiError('No channels found with the provided API key', 404);
        }
        
        // Save the channel connection to the user's profile
        // In a real implementation, this would store the connection in a database
        ...
        console.error('Error connecting to YouTube channel:', error);
        return createApiError(`Error connecting to YouTube channel: ${error.message}`, 500);
      }
    }
